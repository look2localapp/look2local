...
function SearchResults() {
  ...
  const [lockItem, setLockItem] = useState<SearchResult | null>(null);

  const [resultsState, setResultsState] = useState<SearchResult[]>([]);
  ...
  // 2. Fetch search results when query or location changes
  useEffect(() => {
    if (!query.trim()) {
      setResultsState([]);
      setLoading(false);
    ...
    const fetchResults = async () => {
      ...
      try {
        ...
        const res = await fetch(`/api/product-search/compare?q=${encodeURIComponent(query)}&lat=${lat}&lng=${lng}`);
        if (!res.ok) {
        ...
        // Flatten the comparisons into the flat search results structure
        const flattened: SearchResult[] = comps.flatMap((comp: any) =>
          comp.shops.map((shopOffer: any) => ({
            shopId: shopOffer.shopId,
            shopName: shopOffer.shopName,
            shopImage: shopOffer.shopImage || "https://images.unsplash.com/photo-1550009158-9effb619a6c4?q=80&w=200&auto=format&fit=crop",
            verified: shopOffer.gstVerified,
            rating: shopOffer.rating,
            reviews: shopOffer.reviewsCount,
            distance: shopOffer.distanceText,
            address: shopOffer.address || "Local",
            googleMapLink: shopOffer.googleMapLink || "#",
            productId: shopOffer.productId,
            title: comp.name,
            price: shopOffer.price,
            offerPrice: shopOffer.offerPrice,
            productImage: comp.image || "https://images.unsplash.com/photo-1695048133142-1a20484d2569?q=80&w=400&auto=format&fit=crop",
            inStock: true,
            cardOffers: shopOffer.cardOffers || [],
          }))
        );

        setResultsState(flattened);
      } catch (err: any) {
  ...
  const bestPrice = results.length > 0 ? Math.min(...results.map(r => r.offerPrice)) : 0;
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      ...
      {loading ? (
        <div className="text-center py-24">
          ...
          <p className="text-gray-500 font-semibold">Searching nearby stores for "{query}"...</p>
        </div>
      ) : error ? (
